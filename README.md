...
**v0.14.1** — **mobile layout fix.** On narrow phones the multi-point church cost-share table could overflow the right edge of the card, clipping the input fields. The church-name input has an intrinsic minimum width (from the browser's default field size) that `width:100%` can't shrink, so the table's minimum width exceeded a small phone's content area. Fixed on mobile only (`@media (max-width: 580px)`) by letting the name field shrink (`min-width: 0`), trimming the Amount column, and tightening cell padding — the table now fits down to ~320px-wide screens. The desktop layout is unchanged.

**v0.14.0** — replaced the reverse-mode "flat amount per church" option with a single **linked cost-share table**. In max-salary mode, a multi-point charge shows one table where each church's **percentage and dollar amount stay in sync**: edit an amount and the total budget becomes the sum of amounts (percentages recompute); edit a percentage and its amount becomes % × budget (budget unchanged); edit the budget and the amounts rescale. Both workflows now share a consistent **Church / Share % (/ Amount) table** and a **two-tone "Even split / Custom split" segmented toggle** — this replaces both the old forward-mode even/custom radios and the reverse-mode single Even-split button. The active side is highlighted, **Even is the default**, and editing any share flips it to Custom; the toggle spans the full column beside "Number of Churches" and matches the input height. Church-name fields **stretch to the full form width**, the **`%` sign sits inside** each Share % input, and **typed percentages are capped to two decimals** (percentages *derived* from a dollar-amount edit stay exact, so splits still total 100% and reconcile to the penny). Percentages remain strictly enforced to total 100%, and per-church figures stay penny-exact. Also moved the **Churches section directly below the Salary/Budget field**, set the **MCAA control to full width** so its label no longer crowds the church-count input, and updated the pastor-name placeholder to Morgan Lee. Each church's **cost share now appears in the results-table column header** as `Church Name (NN%)` (on screen and in the copied table), and a zero **Amount** field in the reverse-mode table shows a **`$0` placeholder** that clears on click instead of a literal `$0` you must delete.
